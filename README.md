...
<img width="466" height="167" alt="image" src="https://github.com/user-attachments/assets/eeffa7f2-3f06-43c2-8902-64a2857e444a" />
<img width="759" height="410" alt="image" src="https://github.com/user-attachments/assets/ec7afbe0-ef09-4d61-bae0-94e753fc7bb6" />
<img width="534" height="365" alt="image" src="https://github.com/user-attachments/assets/3b36fd07-3a6a-4084-9775-48078f64b679" />
<img width="511" height="375" alt="image" src="https://github.com/user-attachments/assets/3af9396c-fa21-45d1-b08c-cd05cec159e0" />


The surrogate models are evaluated on the same preprocessed dataset as the that used to evaluate black-box models. As a remider, the ROC_AUC score (Receiver Operating Characters Area Under the Curve) is a metric used to evaluate the performance of a binary classification model by showing the model's ability to differentiate between two classes across all possible classification thresholds. The ROC_AUC score of 1 means the classifier is perfect and 0.5 means a random classifier. Values between 0.5 and 1.0: Represent the model's ability to distinguish between the classes, with higher values indicating better performance. In this case, the NaiveBayes model is the highest with the ROC_AUC value of (0.76) and the Dicision trees model with the lowest ROC_AUC score of (0.57) with Logistric Regression (0.75), K-nearest Neighbor (0.71) and Support vector Machines (0.65) respectefully complete the performance hierachy based on the ROC_AUC score. Please note that the score are rounded up to nearest two decimal places.

The accuracy score is an evaluation metric of a percentage measure that tells how often a model can correctly predict an outcome. In this case, the interpretable model with the highest accuracy score is the decision trees (0.89) the followed by the k-nearest neighbor (0.79), then Logistic Regression with (0.77), followed by the Support vector Machines with (0.77) and lastly the Naive Bayes with (0.73).

Precision is a metric that measures the accuracy of positive predictions of a model. All these interpretable models are good in predicting class (0) that is an individual does not have stroke over balanced data as they scored (0.97) all of them except Dicision trees with (0.95). This models struggle in correctly predicting the minority class (1) which is an individual has stroke. Based on the precision scores recorded, we can judge the Support Vector Machine and K nearest neighbors with (0.52) are the highest followed by Dicision trees (0.13), then Logistices regression with (0.12) and lastly Naive Bayes with (0.12).
Recall is a metric that measures the ability of a model to correctly identify all relevant instances or positive cases in a dataset. A high recall score shows that the model is effective at finding most of the positive instances, minimizing the number of missed positive cases (false negatives). All these interpretable models are good in predicting class (0) that is an individual does not have stroke over balanced data as they scored (0.90) for Dicision trees, then KNN with (0.80), followed by Logistic Regression with (0.79), then SVM with (0.78) and lastly Naive Bayes with (0.74). This models struggle in correctly predicting the minority class (1) which is an individual has stroke. Based on the recall scores recorded, we can judge the Naive Bayes as the highest with (0.61), then the Logistices regression with (0.50) followed by Dicision trees (0.24) then K nearest neighbors with (0.13) ans lastly Support Vector Machine with (0.12). A higher F1 score (closer to 1) indicates better model performance, signifying a good balance between precision and recall. Since STOTE was used to balance the data, F1 scores will note be important for judging performance.
